...
static int aw9523_pinctrl_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(aw9523_pin)) {
		...
		AW9523_LOG("%s devm_pinctrl_get fail!\n", __func__);
	}
	...
	if (IS_ERR(shdn_high)) {
		...
		AW9523_LOG("%s : pinctrl err, aw9523_shdn_high\n", __func__);
	}

	shdn_low = pinctrl_lookup_state(aw9523_pin, "aw9523_shdn_low");
	if (IS_ERR(shdn_low)) {
		ret = PTR_ERR(shdn_low);
		AW9523_LOG("%s : pinctrl err, aw9523_shdn_low\n", __func__);
	}
...
static enum hrtimer_restart aw9523_key_timer_func(struct hrtimer *timer)
{
	...
	if(aw9523_key!=NULL){
		...
		//AW9523_LOG("[aw9523_key_timer_func]aw9523_key->is_screen_on = %d\n",aw9523_key->is_screen_on);
	}
...
static void gpio_main_pwm_keyboardlight_100(void)
{
	...
	aeon_gpio_set("keyboardlight_gpio_pwm");
	pwm_setting.pwm_no	= 0;
...
static void gpio_main_pwm_keyboardlight_0(void)
{
	...
	pwm_set_spec_config(&pwm_setting);
#else
	aeon_gpio_set("keyboardlight_gpio_low");
#endif
}
...
static ssize_t aw9524_led_proc_fops_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	//reg_val_0 = i2c_read_reg(P0_OUTPUT);
	//AW9523_LOG("start P0_OUTPUT_AW9524 reg_val_0 = 0x%2X\n", reg_val_0);
	reg_val_1 = i2c_read_reg(P1_OUTPUT);
	AW9523_LOG("start P1_OUTPUT reg_val_1 = 0x%2X\n", reg_val_1);
	AW9523_LOG("start  str_buf = 0x%d\n", str_buf[2]);
	if (str_buf[0]== '1') {
		if (str_buf[1]== '1') {
			//i2c_write_reg(P0_0_DIM0_AW9524,(((int)str_buf[2]-48)*brightness_red));
		} else if (str_buf[1]== '2') {
			//i2c_write_reg(P0_1_DIM0_AW9524,(((int)str_buf[2]-48)*brightness));
		} else if (str_buf[1]== '3') {
	...
	//reg_val_0 = i2c_read_reg(P0_OUTPUT);
	//AW9523_LOG("end P0_OUTPUT_AW9524 reg_val_0 = 0x%2X\n", reg_val_0);
	
	reg_val_1 = i2c_read_reg(P1_OUTPUT);
	AW9523_LOG("end P1_OUTPUT reg_val_1 = 0x%2X\n", reg_val_1);  
    AW9523_LOG("%s \n", str_buf);	
    return count;
}
...
static int aw9523_key_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		AW9523_LOG("[%s] failed to init aw9523 pinctrl.\n", __func__);
		return ret;
	} else {
	...
	if (ret != 0) {
		AW9523_LOG("[%s] failed to register aw9523 i2c driver.\n", __func__);
		return ret;
	} else {
...
static int __init aw9523_key_init(void) {
	...
	if (ret) {
		AW9523_LOG("****[%s] Unable to register driver (%d)\n", __func__, ret);
		return ret;
	}		
	return 0;
}
